...
const AdminDashboard: React.FC = () => {
  ...
  const fetchGalleryStats = async () => {
    try {
      const galleryItems = await AdminGalleryService.getAllGalleryItems();
      setGalleryStats({
      ...
      setGalleryStats(prev => ({ ...prev, loading: false }));
    }
  ...
  const handleUploadMedia = () => {
    ...
    },
    {
      ...
      href: '/admin/gallery',
      stats: { 
        label: galleryStats.loading ? 'Yükleniyor...' : 'Toplam Görsel', 
        value: galleryStats.loading ? '-' : galleryStats.total.toString()
      }
    }
  ];const quickActions = [
    ...
    { title: 'Medya Yükle', icon: Images, action: handleUploadMedia, color: 'bg-pink-500' }
  ];const recentStats = [
    { 
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <Link
                to="/"
                className="flex items-center space-x-2 px-3 py-2 bg-muted hover:bg-muted/80 rounded-lg transition-colors duration-200 text-muted-foreground hover:text-foreground"
              >
